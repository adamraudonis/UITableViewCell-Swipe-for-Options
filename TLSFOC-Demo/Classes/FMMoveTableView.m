...
- (void)prepareGestureRecognizer
{
    ...
    [self.movingGestureRecognizer setMinimumPressDuration:.1];
...
- (void)handleTap:(UITapGestureRecognizer *)gestureRecognizer
{
    CGPoint touchPoint = [gestureRecognizer locationInView:self];
    NSIndexPath *touchedIndexPath = [self indexPathForRowAtPoint:touchPoint];
    BOOL shouldBegin = [self isValidIndexPath:touchedIndexPath];
    
    if (shouldBegin && [self.delegate respondsToSelector:@selector(moveTableView:didSelectRowAtIndexPath:)]) {
        //[self selectRowAtIndexPath:touchedIndexPath animated:YES scrollPosition:UITableViewScrollPositionNone];
        [self.delegate moveTableView:self didSelectRowAtIndexPath:touchedIndexPath];
    }
    NSLog(@"INSIDE HANDLE TAP");
}
...
- (UIView *)snapShotFromRowAtMovingIndexPath
{
    ...
    TLSwipeForOptionsCell *touchedCell = (TLSwipeForOptionsCell *)[self cellForRowAtIndexPath:self.movingIndexPath];
